...
rosen_trts = {
    ...
    "weather_derived_res": [
        ...
        "precip_cumsum_plant_to_date",
        "et_rs_cumsum_plant_to_date",
        "solar_rad_cumsum_plant_to_date",
        "gdd_diff_cumsum_plant_to_date",
        "precip_diff_cumsum_plant_to_date",
        ...
        "gdd_cumsum_last14",
        "precip_cumsum_last14",
        "et_rs_cumsum_last14",
        "solar_rad_cumsum_last14" "gdd_diff_cumsum_last14",
        "precip_diff_cumsum_last14",
        "et_rs_diff_cumsum_last14",
        "solar_rad_diff_cumsum_last14",
        "temp_diff_cummean_last14",
...
nni = {
    ...
    "wx": [
        ...
        "et_rs_cumsum_plant_to_date",
        "precip_diff_cumsum_plant_to_date",
        "et_rs_diff_cumsum_plant_to_date",  # 2nd
        "ipar_cumsum_plant_to_date",
        "temp_diff_cummean_bulk_to_date",
        "et_rs_cumsum_bulk_to_date",
        "solar_rad_cumsum_bulk_to_date",
        "ipar_diff_cumsum_plant_to_date",
        "temp_diff_diff_cummean_bulk_to_date",
        "et_rs_diff_cumsum_bulk_to_date",
        "solar_rad_diff_cumsum_bulk_to_date",
    ],
}
...
biomass = {
    ...
    "wx": [
        "gdd_cumsum_plant_to_date",
        "ipar_cumsum_plant_to_date",  # 1st
        "temp_diff_cummean_bulk_to_date",
        "et_rs_cumsum_bulk_to_date",
        "solar_rad_cumsum_bulk_to_date",  # 2nd
        "precip_cumsum_plant_to_date",
        "et_rs_cumsum_plant_to_date",
    ],
    "cropscan_wl_range1": [400, 900],
...
config_dict = {
    "Tables": {
        ...
        "table_names": {  # if not connected to a DB, these should point to files that contain the join data.
            ...
            "obs_tissue_res": "obs_tissue_research.csv",
            "obs_soil_res": "obs_soil_research.geojson",
            "rs_cropscan_res": "rs_cropscan_res.csv",
    ...
    "FeatureData": {
        "random_seed": 999,
        "dir_results": None,
        "response_data": {  # must include "table_name" and "value_col" (required), as well as key/value pairs to pass to DB_handler.get_table_df()
        ...
        "cv_method_kwargs": {
            "n_groups": 1
        },  # will be passed as ['cv_method'](**['cv_method_kwargs'])
        "cv_split_kwargs": {
            "groups": 'df["year"] != 2020'
        },  # explicitly places <groups> == True (e.g., 2018 and 2019) in train set
        # 'cv_method': LeavePGroupsOut,
    ...
    "FeatureSelection": {
        ...
        "model_fs_params_adjust_max": {
            "alpha": 1e-3
        },  # the search for the range of parameters
        "n_feats": 12,
        "n_linspace": 150,
