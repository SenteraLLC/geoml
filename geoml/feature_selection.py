...
class FeatureSelection(FeatureData):
    ...
    def _set_model_fs(self):
        ...
        if "regressor" in self.model_fs.get_params().keys():
            ...
            try:
                self.model_fs.set_params(
                    **{regressor_key_fs + "random_state": self.random_seed}
                )
            except ValueError:
                print(
        ...
        if feat_n_sel <= 1:  # the initial value already results in 1 (or 0) feats
            while feat_n_sel <= 1:
                ...
                self.model_fs_params_adjust_min = self._params_adjust(
                    self.model_fs_params_adjust_min,
                    key="alpha",
                    increase=False,
                    factor=1.2,
                )
    ...
    def _lasso_fs_df(self):
        ...
        if self.n_feats > 1:
            ...
            param_val_list = list(
                np.logspace(params_min, params_max, num=self.n_linspace, base=np.e)
            )
        else:
            ...
            params_min = np.log(self.model_fs_params_feats_min["alpha"])
            param_val_list = list(
                np.logspace(params_min, params_max, num=self.n_linspace, base=np.e) / 10
            )
        ...
        msg = (
            "The alpha value that achieves selection of {0} feature(s) was "
    ...
    def fs_find_params(self, **kwargs):
        ...
        if self.n_feats > self.X_train.shape[1]:
            print(
                "``n_feats`` must not be more than are available.\n"
        ...
        # elif self.model_fs_name == 'PCA':
            ...
            raise NotImplementedError(
                "{0} is not implemented.".format(self.model_fs_name)
            )
        # else:
    ...
    def fs_get_X_select(self, df_fs_params_idx):
        ...
        """
        msg1 = (
            "<df_fs_params> must be populated; be sure to execute "
        ...
        self.labels_x_select = self.df_fs_params.loc[df_fs_params_idx][
            "labels_x_select"
        ]
