...
def test_training_init_fixture():
    my_tune = Training(param_dict=feature_groups.param_dict_test, print_out=False)
    return my_tune
...
#     my_tune = Training(param_dict=feature_groups.param_dict_test,
...
class Test_training_self:
    ...
    def test_kwargs_override_print_out_train(self):
        ...
        my_tune.train()  # for test coverage on print_out_train=True
        assert my_tune.print_out_train == print_out_train
...
class Test_training_self_simple:
    def test_kwargs_override_regressor(self):
        ...
        param_dict_tune = {
            ...
            'print_out_train': True}
        my_tune = Training(param_dict=param_dict_tune, base_dir_data=base_dir_data)
        assert my_tune.print_out_train == True

class Test_training_df_tune_scores:
    def test_scoring_len(self, test_training_train_fixture):
        my_tune = test_training_train_fixture
...
class Test_training_df_test_scores:
...
class Test_training_df_tune_params:
    def test_df_tune_params_reg_tune_match1(
        ...
        my_tune = test_training_train_fixture
        for idx, row in my_tune.df_tune.iterrows():
...
class Test_training_set_kwargs:
    ...
    def test_set_kwargs_param_dict_none(self):
        base_dir_data = feature_groups.param_dict_test['FeatureData']['base_dir_data']
        my_tune = Training(param_dict=None, base_dir_data=base_dir_data)
    ...
    def test_set_kwargs_regressor(self):
        ...
        my_tune = Training(param_dict=feature_groups.param_dict_test,
                         ...
                         param_grid=param_grid)
        my_tune.train()
        assert 'PLSRegression' in my_tune.df_test_filtered['regressor_name'].unique()
    ...
    def test_set_kwargs_regressor_two_models(self, test_training_train_fixture):
        ...
        my_tune.train(regressor=regressor,
                               regressor_params=regressor_params,
                               param_grid=param_grid)
        assert 'Lasso' in my_tune.df_test_filtered['regressor_name'].unique()
        assert 'PLSRegression' in my_tune.df_test_filtered['regressor_name'].unique()
